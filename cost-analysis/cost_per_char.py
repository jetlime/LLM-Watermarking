...
def cost(characters_amount, model):
    ...
    return (total_input_tokens * price_per_token[model][0]) + (
        total_output_tokens * price_per_token[model][1]
    )
...
ax.axvline(
    avg_characters_academic_paper,
    color="red",
    linestyle="--",
    label="Avg Academic Paper",
)
...
def cost_fixed_sequence_window(characters_amount, model, window_size):
    ...
    total_tokens = avg_token_per_char * characters_amount
    total_output_tokens = total_tokens
    total_input_tokens = (total_tokens / window_size) * (
