...
def iterative_word_predition(raw_tokens, encoder):
    ...
    for i in tqdm(range(1, len(raw_tokens))):
        ...
        for next_word_candidate in next_word_log_probs:
            index+=1
            if [next_token] == encoder.encode(next_word_candidate, allowed_special={'<|endoftext|>'}):
...
plt.title(f"Total API cost U.S ${round(request_cost, 3)} for a document of {number_char} characters")
